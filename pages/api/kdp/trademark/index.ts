...
async function GetGPTResponse(
  ...
  try {
    const behaviouralPrompt = `
      Check the following field for any trademark violations:
      Field: ${fieldName}
      Content: ${fieldContent}

      Please list any potential trademark violations and format the response like:
      {
        "isViolated": boolean,
        "violations": Violated[] | null // array of fields that are violated and the reasons for it like array of {"fieldname": string, "reason":string, "violation":string, "violatedPart" :string, "severity" : "red" | "yellow"} or null if none
      }
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (bookType) fieldChecks.push(checkField("Book Type", bookType));
